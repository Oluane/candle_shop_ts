      ...
      {wishlist.products[0].candleId !== -1 ? (
        <div className="wishlistWrapper">
            ...
            return (
              (i + 1 <= itemsToDisplayOnInit || displayAllItems) && (
                <div className="wishlistRow" key={"wishlistRow" + i}>
                  {deviceWidth > 688 && (
                    <div className="trashCan" onClick={() => deleteCandleFromWishlist(product.candleId)}>
                  ...
                  <div className="wishlistProductInfos">
                    ...
                    <div className="candleInfoContainer mediumText">
                      <p className="mediumBold">{product.price.toFixed(2)}€</p>
                      <p className="separatorBefore">{product.sizeEnName}</p>
                      <p className="separatorBefore">{product.weightInGr}gr</p>
                      <p className="separatorBefore">{product.durationInHours}h</p>
                    </div>
                  </div>
                  {deviceWidth > 688 ? (
                    <AddToCartBtn
          ...
          {wishlistProducts.length < itemsToDisplayOnInit ? (
            ...
            !displayAllItems && (
              <button className="wishlistBtns smallText mediumBold" onClick={() => setDisplayAllItems(true)}>
                SHOW ALL CANDLES
              </button>
            )
          )}
