...
const AddToWishlistBtn = ({ typeSize, scent }) => {
  ...
  const addCandleToWishlist = () => {
    const { typeSizeId, durationInHours, price, sizeEnName, typeEnName, typeId, weightInGr } = typeSize;
  ...
  const isCandlePresentInWishlist = () => {
    ...
    if (wishlistProducts.findIndex((candle) => candle.typeSizeId === typeSizeId && candle.scentId === scentId) === -1) {
      return false;
    } else {
      return true;
    }
  ...
  return (
    ...
    >
      <div
        className={"addToWishlistIcon" + (scent !== undefined ? (isCandlePresentInWishlist() ? " disabled" : "") : "")}
      >
