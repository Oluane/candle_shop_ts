	...
	switch (action.type) {
		...
		case addressesActions.ADDRESSES_DELETE_ONE.type:
			...
			let newStateAfterDelete = state.filter(
				(address) => address.id !== action.payload.deletedAddressId
            );
            
            if (action.payload.nextFavAddressId){
                const nextFavAddressIdx = newStateAfterDelete.findIndex((address)=> address.id === action.payload.nextFavAddressId);
